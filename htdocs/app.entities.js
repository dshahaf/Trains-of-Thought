...
var multiFillText = function(ctx,text, x, y, lineHeight, fitWidth) {
    ...
    var printNextLine = function(str) {
        ...
        if (draw) {
            ctx.fillText(str, x-wordWidth/2, y + (lineHeight * (currentLine-numLines+1)));
        }
...
var multiMeasureText = function(text, lineHeight, fitWidth) {
    ...
    var background = function() {
        ...
        that.draw = function(ctx, renderer) {
            ...
            if (!this.cache || _imagedataWidth != ww || _imagedataHeight != wh) {
                ...
                grad.addColorStop(1, '#bdedff'); //'#ffffff'); //
    ...
    var edge = function(spec) {
        ...
        that.draw = function(ctx) {
			...
			// !D
            var relationColor = 'rgb(0, 0, 0)';
			//source.selected ? 'rgb(255, 255, 255)' : 'rgba(255, 255, 255, 0.5)';
            ...
            var arrowWidth = 0;
            var arrowHeight = 0;
            ctx.lineWidth = (arrowWidth / 10) * this.weight + 4;
	     ...
	     {
	       ctx.strokeStyle = ctx.fillStyle = this.lines[i].color;
		ctx.translate(4*i,0 );
            	ctx.beginPath();
            	ctx.moveTo(0, 0);
	     ...
	     // !D second line. Figure perpendicular
	     //var perpangle = Math.PI/2 - angle;
	     //if (perpangle<0) perpangle+= Math.PI*2;
	     //var targetperpAngle = app.math.getOpositeTriangleRadian(perpangle );
            //var sourceperpCoord = app.math.getCircleEdge(4, perpangle); //source.getEdge(perpangle);
            //var targetperpCoord = target.getEdge(targetperpAngle);


	     
            //ctx.translate(sourceperpCoord.x, sourceperpCoord.y );
	     

	     //ctx.beginPath();
            //ctx.moveTo(0, 0);
            //ctx.lineTo(0, -length + arrowHeight);
	     //ctx.lineTo(sourceperpCoord.x , -length + arrowHeight + sourceperpCoord.y);
            //ctx.stroke();
    ...
    var dragable = function() {
		...
		$('#'+this.id+'dial').dialog('close');
	      	$('#'+this.id+'modal').reveal();
        ...
        that.addEventListener('mouseover', function(e){
          //var ent= ns.world.getEntityByWindowCoord(e.pageX, e.pageY);
	   //var pos= ns.world.getViewCoord(e.pageX, e.pageY, ent.view)
	   $('#'+this.id+'dial').dialog({position: [e.pageX + 20, e.pageY - 30], height: 300, maxHeight: 350}); 
          $('#'+this.id+'dial').dialog('open'); 
	   window.lastDialog= this.id;
	   /*$('#'+this.id+'dial').mouseleave(function (e) { 
	      		$('#'+e.currentTarget.id).dialog('close');
	    		});
	   */
        });
    ...
    var node = function(nospec, our) {
        ...
        that.draw = function(ctx, renderer) {
            ...
            if ($.browser.mozilla && !ctx.fillText) {
                ctx.save();
                ctx.fillStyle = this.selected ? '#FF006A' : 'rgba(0, 0, 0, 0.6)'; // ! D
                ctx.mozTextStyle = '11px Verdana';
                ctx.translate(x, y);
                ctx.mozDrawText(this.label || '');
                ctx.restore();
            } else {
                ctx.fillStyle = this.selected ? '#FF006A' : 'rgba(0, 0, 0, 0.8)'; // ! D
                ctx.font = '11px Verdana';
                //ctx.fillText(this.label || '', x, y);
				multiFillText(ctx, this.label + "\n" + this.date, x, y, 15, cutOff);
            }
        };
    ...
    var circle = function() {
        ...
        that.draw = function(ctx) {
            ...
            ctx.fillStyle = 'rgba(255, 255, 255, 0.8)';
			   //this.selected ? 'rgba(63, 115, 157, 0.8)' : 'rgba(17, 31, 41, 0.5)';
            ctx.lineWidth = 1.5;
            ctx.strokeStyle = this.found ? '#D5E625' : this.selected ? 'rgb(0, 0, 0)' : 'rgb(0, 0, 0)'; //'rgba(0, 0, 0, 0.5)';
    ...
    var tools = function() {
        ...
        that.draw = function(ctx, renderer) {
	     ...
	     ctx.strokeStyle = 'rgba(150, 150, 150, 0.9)';
            ctx.lineWidth = 15;
            ctx.lineCap = 'round';
            ctx.moveTo(br.x - 160, br.y - 20);
            ctx.lineTo(br.x - 20, br.y - 20);
            ctx.stroke();
	     ctx.fillStyle = 'rgba(150, 150, 150, 0.9)';
	     ctx.fillText('Zoom', br.x-105, br.y-30);

	     ctx.fillStyle = 'rgba(255, 255, 255, 0.9)';
	     ctx.fillText('-', br.x-160, br.y-18);
	     ctx.fillText('+', br.x-23, br.y-16);

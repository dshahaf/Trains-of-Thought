...
(function(ns){
    ...
    var runtime = function() {
        ...
        var buildEntitiesFromModel = function(zIndex) {
			...
			var i;
			for (i=0; i<_nodes.Keys.length; i++) {
				var tNode = _nodes.Lookup(_nodes.Keys[i]);
				var node = ns.entities.circle();
	             
                node.id = tNode.id;
                node.label = tNode.label;
				node.date= tNode.date;
				node.ourx = tNode.x;
				node.oury = tNode.y;
				node.__model = this;
				
				node.setPosition(node.ourx * ns.world.window.width , node.oury * ns.world.window.height);
			...
			var firstOcc= node.text.indexOf("<a href=\"#secondParagraph");
			var secOcc= node.text.lastIndexOf("<a name=\"secondParagraph");
			
			if (firstOcc > -1 && secOcc > firstOcc)
			{
				// found it
				var nextTag= node.text.indexOf("<", secOcc+1);
				node.text= node.text.substring(0,firstOcc) + node.text.substring(nextTag);
			}
			

			var stLong= node.text; 
			firstOcc= node.text.indexOf("<div class=\"articleBody\">");
			secOcc= node.text.indexOf("</div>",firstOcc);
			if (firstOcc > -1)
			{
				if (secOcc > firstOcc)
					node.text= node.text.substring(firstOcc, secOcc + 6);
				else
				       node.text= node.text.substring(firstOcc) + "</div>";

			}
			
			// get rid of other links
			node.text = node.text.replace(/a href/gi, "/i ");
			node.text = node.text.replace(/a name/gi, "/i ");
			node.text = node.text.replace(/img /gi, "/i ");
			node.text = node.text.replace("class=\"inlineImage\"", "class=\"inlineImage\" style=\"display: none;\"");
			node.text = node.text.replace("class=\"inlineViedo\"", "class=\"inlineVideo\" style=\"display: none;\"");
			node.text = node.text.replace("class=\"mediaOverlay\"", "class=\"mediaOverlay\" style=\"display: none;\"");
			node.text = node.text.replace("class=\"articleInline\"", "class=\"articleInline\" style=\"display: none;\"");

			stLong = stLong.replace(/a href/gi, "/i ");
			stLong = stLong.replace(/a name/gi, "/i ");
			...
			dial.setAttribute("style", "display:none;overflow:hidden");
			...
			modal.innerHTML = "<h1>"+node.label+"</h1>"+ stLong ;
			...
			var j, k;
			for (j=0; j<_edges.length; j++) {
				var Edge = _edges[j];
				var source = findNodeById(Edge.source.id);
				var target = findNodeById(Edge.destination.id);
				var edge = ns.entities.edge({source: source, target: target, weight : 1, lines: Edge.lines});
        ...
        var setupHIDHandling = function() {
		  ...
		  if (!ent)
		  {
			...
			{	
				$('#'+window.lastDialog+'dial').dialog('close'); 
        ...
        that.init = function(element) {
            ...
            var updateDim = function() {
                //var w = $(window).width()*0.7; // !D
                //var h = $(window).height()*0.7;

			var xdelta= $('.map1').offset().left ;
			var ydelta= $('.map1').offset().top;
			
			
			
				var w = $('.map1').width() + xdelta; // !D
                var h = $('.map1').height() + ydelta;
        ...
        that.start = function(xNodes, xEdges, xLines) {
			...
			window._nodes= xNodes;
			window._edges= xEdges;
			window._lines= xLines;
